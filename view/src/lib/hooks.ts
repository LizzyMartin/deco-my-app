...
/**
 * IBGE Integration Hooks
 */

export const useListBrazilianStates = () => {
  return useSuspenseQuery({
    queryKey: ["brazilian-states"],
    queryFn: () => client.LIST_BRAZILIAN_STATES({}),
    staleTime: 1000 * 60 * 60, // Cache for 1 hour since states don't change frequently
  });
};

export const useFetchBrazilianStatesWorkflow = () => {
  const queryClient = useQueryClient();
  return useMutation({
    mutationFn: () => client.FETCH_BRAZILIAN_STATES_WORKFLOW({}),
    onSuccess: () => {
      // Invalidate the states query to refresh data
      queryClient.invalidateQueries({ queryKey: ["brazilian-states"] });
      toast.success("Estados brasileiros carregados com sucesso!");
    },
    onError: (error) => {
      toast.error(`Erro ao carregar estados: ${error.message}`);
    },
  });
};
