...
/**
 * IBGE Integration Tools
 * Tool to fetch Brazilian states from IBGE public API
 */
export const createListBrazilianStatesTool = (env: Env) =>
  createTool({
    id: "LIST_BRAZILIAN_STATES",
    description: "List all Brazilian states from IBGE API",
    inputSchema: z.object({}),
    outputSchema: z.object({
      states: z.array(
        z.object({
    ...
    execute: async () => {
      try {
        const response = await fetch(
          "https://servicodados.ibge.gov.br/api/v1/localidades/estados",
          {
            headers: {
              ...
              "User-Agent": "Deco-IBGE-Integration/1.0",
            },
          }
        );
        ...
        // Validate and sort states by name
        const sortedStates = states
          .filter((state: any) => state && state.id && state.nome && state.sigla)
        ...
        throw new Error(`Failed to fetch Brazilian states: ${error instanceof Error ? error.message : 'Unknown error'}`);
      }
    },
  });
